...
This application also includes several variations of a **client-side Web service** to ensure a functional example. These **service-clients** enable end users to submit salutation requests to the server-side Web service. Refer to the [Client-Side Flows](https://docs.mulesoft.com/mule-runtime/3.5/soap-web-service-security-example#client-side-flows) section below for more details.

This document helps you understand some of the ways you can apply security to a SOAP Web service in Mule ESB applications. To understand more about Mule ESB’s ability to integrate services and systems, access the [Mule examples](https://docs.mulesoft.com/mule-runtime/3.5/mule-examples) and see other applications in action.
...
1. [**Create, then run**](https://docs.mulesoft.com/mule-runtime/3.5/mule-examples) the example application in Anypoint Studio or Standalone.
...
1. Complete the [procedure](https://docs.mulesoft.com/mule-runtime/3.5/mule-examples) to create, then run the Security template in Anypoint Studio, or the Security example in Mule Standalone (Mule ESB without Studio).
...
![unsecureService](https://docs.mulesoft.com/mule-runtime/3.5/_images/unsecureservice.png)
...
![unsecuresoap](https://docs.mulesoft.com/mule-runtime/3.5/_images/unsecuresoap.png)
...
**Where is the Java code?**To access the Java code in **Anypoint Studio**, navigate to the source file in the **Package Explorer**.**Find the Java:** . In your WS-Security project, navigate to `src/main/java` > `com.mulesoft.mule.example.security`. . Double-click the `Greeter.java` file to open it in a new tab on the Studio canvas (see image below).+ image::greeter.png[]+ To access the Java code in **Mule Standalone**, navigate to the source file in the example folder on your local drive.+ . Navigate to the Mule Standalone folder on your local drive. . Navigate to `examples` > `security` > `src` > `main` > `java` > `com` > `mulesoft` > `mule` > `example` > `security`. . Double-click to open the `Greeter.java`file.
...
![usernameTokenService](https://docs.mulesoft.com/mule-runtime/3.5/_images/usernametokenservice.png)
...
![soap2](https://docs.mulesoft.com/mule-runtime/3.5/_images/soap2.png)
...
![usernameTokenSigned](https://docs.mulesoft.com/mule-runtime/3.5/_images/usernametokensigned.png)
...
![soap3](https://docs.mulesoft.com/mule-runtime/3.5/_images/soap3.png)
...
**Where is the wssecurity.properties File?**To access the **wssecurity.properties** file in Studio, navigate to the source file in the **Package Explorer**.In your WS-Security project, navigate to `src/main/resources`.Double-click the `wssecurity.properties` file to open it in a new tab on the Studio canvas (see image below).   image::ws-security-properties.png[ws_security_properties]
...
![UsernameTokenEncrypted](https://docs.mulesoft.com/mule-runtime/3.5/_images/usernametokenencrypted.png)
...
![soap4](https://docs.mulesoft.com/mule-runtime/3.5/_images/soap4.png)
...
**How is the Message Body Encrypted?**To demonstrate a functional example, the WS-Security application includes several **client-side flows** which provide the **server-side flows** with security information.The **CXF Component** in the client-side **usernameTokenEncrypted** subflow encrypts the body of the SOAP message (see image below).![subflow2](https://docs.mulesoft.com/mule-runtime/3.5/_images/subflow2.png)![2encrypt](https://docs.mulesoft.com/mule-runtime/3.5/_images/2encrypt.png)Refer to the [Client-Side Flows](https://docs.mulesoft.com/mule-runtime/3.5/soap-web-service-security-example#client-side-flows) section below for more details.

### SAMLTokenService Flow

#### Studio Visual Editor

![samlToken](https://docs.mulesoft.com/mule-runtime/3.5/_images/samltoken.png)
...
![samldelegate](https://docs.mulesoft.com/mule-runtime/3.5/_images/samldelegate.png)

![samlge](https://docs.mulesoft.com/mule-runtime/3.5/_images/samlge.png)
...
**For Mule Studio Users**To access the **SAMLCustomValidator** Java class in Studio, navigate to the source file in the **Package Explorer**.In your WS-Security project, navigate to `src/main/java` > `com.mulesoft.mule.example.security`.Double-click the `SAMLCustomValidator.java` file to open it in a new tab on the Studio canvas (see image below).![SAMLCustomValidator](https://docs.mulesoft.com/mule-runtime/3.5/_images/samlcustomvalidator.png)

**How Does the Web Service Client Send a SAML2 Assertion?**To demonstrate a functional example, the WS-Security application includes several **client-side flows** which provide the **server-side flows** with security information.The **CXF Component** in the client-side **samlToken** subflow uses a SAMLCallbackHandler (see Java code below this box) to populate the assertion it sends to the Web Service. (see image below).![saml7](https://docs.mulesoft.com/mule-runtime/3.5/_images/saml7.png)
...
Refer to the [Client-Side Flows](https://docs.mulesoft.com/mule-runtime/3.5/soap-web-service-security-example#client-side-flows) section below for more details.

### SignedSAMLTokenService Flow

#### Studio Visual Editor

![signedSamlToken](https://docs.mulesoft.com/mule-runtime/3.5/_images/signedsamltoken.png)
...
Much the same as [SAMLTokenService Flow](https://docs.mulesoft.com/mule-runtime/3.5/soap-web-service-security-example#samltokenservice-flow), this flow uses `SAMLTokenUnsigned` action to validate SAML2 assertions. However, the action in this flow uses a digital signature to verify the trustworthiness of the original authenticator. To validate the digital signature on the message, the CXF component uses a keystore specified in the **signaturePropFile**. (Refer to [UsernameTokenEncryptedService Flow](https://docs.mulesoft.com/mule-runtime/3.5/soap-web-service-security-example#usernametokenencryptedservice-flow)section above for more information on verifying digital signatures.)
...
![signature](https://docs.mulesoft.com/mule-runtime/3.5/_images/signature.png)
...
![client-server_diagram](https://docs.mulesoft.com/mule-runtime/3.5/_images/client-server-diagram.png)
...
![ws security](https://docs.mulesoft.com/mule-runtime/3.5/_images/ws-security.png)
...
- For more information on the SOAP Component, see [CXF Component Reference](https://docs.mulesoft.com/mule-runtime/3.5/cxf-component-reference).
- For more information on WS-security, see [Enabling WS-Security](https://docs.mulesoft.com/mule-runtime/3.5/enabling-ws-security).
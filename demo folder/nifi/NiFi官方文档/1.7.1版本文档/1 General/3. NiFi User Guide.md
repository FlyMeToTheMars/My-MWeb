...
- Versioning a DataFlow
  - [Connecting to a NiFi Registry](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#connecting-to-a-nifi-registry)
...
See the [System Administrator’s Guide](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html) for information about system requirements, installation, and configuration. Once NiFi is installed, use a supported web browser to view the UI.
...
![Remote Process Group Menu](https://nifi.apache.org/docs/nifi-docs/html/images/nifi-rpg-menu.png)
...
![Primary Node Processors in Summary Page](https://nifi.apache.org/docs/nifi-docs/html/images/primary-node-processors-summary.png)
...
![Create Service](https://nifi.apache.org/docs/nifi-docs/html/images/create-service-ssl-context.png)
...
For more information, see the [Custom Properties](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html#custom_properties) section in the System Administrator’s Guide.
...
![Controller Services Information Buttons](https://nifi.apache.org/docs/nifi-docs/html/images/controller-services-info-buttons.png)
...
![Configure Reporting Task Settings](https://nifi.apache.org/docs/nifi-docs/html/images/configure-reporting-task-settings.png)
...
![Connection Bend Points](https://nifi.apache.org/docs/nifi-docs/html/images/nifi-connection-bend-points.png)
...
- **Component Counts**: The Component Counts element provides information about how many components of each type exist within the Process Group. The following provides information about each of these icons and their meanings:
  ...
  - ![Running](https://nifi.apache.org/docs/nifi-docs/html/images/iconRun.png) **Running Components**: The number of Processors, Input Ports, and Output Ports that are currently running within this Process Group.
...
- **Status History**: Clicking the Status History icon will open a new dialog that shows a historical view of the statistics that are rendered for this component. See the section [Historical Statistics of a Component](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Status_History) for more information.
...
While the Summary table and the canvas show numeric statistics pertaining to the performance of a component over the past five minutes, it is often useful to have a view of historical statistics as well. This information is available by right-clicking on a component and choosing the "Status History" menu option or by clicking on the Status History in the Summary page (see [Summary Page](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Summary_Page) for more information).

The amount of historical information that is stored is configurable in the NiFi properties but defaults to 24 hours. For specific configuration information reference the Component Status Repository of the [System Administrator’s Guide](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html). When the Status History dialog is opened, it provides a graph of historical statistics:

![Status History](https://nifi.apache.org/docs/nifi-docs/html/images/stats-history.png)
...
The right-hand side of the dialog provides a drop-down list of the different types of metrics to render in the graphs below. The top graph is larger so as to provide an easier-to-read rendering of the information. In the bottom-right corner of this graph is a small handle ( ![Resize](https://nifi.apache.org/docs/nifi-docs/html/images/iconResize.png) ) that can be dragged to resize the graph. The blank areas of the dialog can also be dragged around to move the entire dialog.
...
![Registry Clients Tab](https://nifi.apache.org/docs/nifi-docs/html/images/registry-clients-tab.png)

In the Add Registry Client window, provide a name and URL.

![Add Registry Client Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/add-registry-client.png)

Click "Add" to complete the registration.

![Registry Client Added](https://nifi.apache.org/docs/nifi-docs/html/images/registry-client-added.png)
...
- ![Up to date](https://nifi.apache.org/docs/nifi-docs/html/images/iconUpToDate.png) **Up to date**: The flow version is the latest.
- ![Locally Modified](https://nifi.apache.org/docs/nifi-docs/html/images/iconLocallyModified.png) **Locally modified**: Local changes have been made.
- ![Stale](https://nifi.apache.org/docs/nifi-docs/html/images/iconStale.png) **Stale**: A newer version of the flow is available.
- ![Locally Modified and Stale](https://nifi.apache.org/docs/nifi-docs/html/images/iconLocallyModifiedStale.png) **Locally modified and stale**: Local changes have been made and a newer version of the flow is available.
- ![Sync Failure](https://nifi.apache.org/docs/nifi-docs/html/images/iconSyncFailure.png) **Sync failure**: Unable to synchronize the flow with the registry.
...
![Version States Displayed](https://nifi.apache.org/docs/nifi-docs/html/images/version-states-display.png)

Version state information is also shown in the "Process Groups" tab of the Summary Page.

![Version State in Summary Page](https://nifi.apache.org/docs/nifi-docs/html/images/version-state-summary-page.png)
...
![Import Process Group](https://nifi.apache.org/docs/nifi-docs/html/images/add-process-group-import.png)

Selecting the link will open the Import Version dialog.

![Import Version Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/import-version-dialog.png)
...
![Versioned Flow Imported](https://nifi.apache.org/docs/nifi-docs/html/images/versioned-flow-imported.png)

Since the version imported in this example is the latest version (MySQL CDC, Version 3), the state of the versioned process group is "Up to date" (![Up To Date Icon](https://nifi.apache.org/docs/nifi-docs/html/images/iconUpToDate.png)). If the version imported had been an older version, the state would be "Stale" (![Stale Icon](https://nifi.apache.org/docs/nifi-docs/html/images/iconStale.png)).
...
![Start Version Control](https://nifi.apache.org/docs/nifi-docs/html/images/start-version-control.png)
...
![Save Flow Version Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/save-flow-version-dialog.png)

Select Save and Version 1 of the flow is saved.

![Versioned Process Group](https://nifi.apache.org/docs/nifi-docs/html/images/versioned-process-group.png)

As the first and latest version of the flow, the state of the versioned process group is "Up to date" (![Up To Date Icon](https://nifi.apache.org/docs/nifi-docs/html/images/iconUpToDate.png)).
...
When changes are made to a versioned process group, the state of the component updates to "Locally modified" (![Locally Modified Icon](https://nifi.apache.org/docs/nifi-docs/html/images/iconLocallyModified.png)). The DFM can show, revert or commit the local changes. These options are available for selection in the context menu when right-clicking on the process group:

![Local Changes PG Selected](https://nifi.apache.org/docs/nifi-docs/html/images/local-changes-pg-selected.png)

or when right-clicking on the canvas inside the process group:

![Local Changes Inside PG](https://nifi.apache.org/docs/nifi-docs/html/images/local-changes-pg-inside.png)
...
|      | Variables do not support sensitive values and will be included when versioning a Process Group. See [Variables in Versioned Flows](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Variables_in_Versioned_Flows) for more information. |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
![Show Local Changes Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/show-local-changes-dialog.png)

You can navigate to a component by selecting the "Go To" icon (![Go To](https://nifi.apache.org/docs/nifi-docs/html/images/iconGoTo.png)) in its row.

|      | As described in the [Managing Local Changes](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#managing_local_changes) section, there are exceptions to which actions are reviewable local changes. Additionally, multiple changes to the same property will only appear as one change in the list as the changes are determined by diffing the current state of the process group and the saved version of the process group noted in the Show Local Changes dialog. |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
![Revert Local Changes Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/revert-local-changes-dialog.png)
...
![Save Flow Version Commit](https://nifi.apache.org/docs/nifi-docs/html/images/save-flow-version-commit.png)

Local changes can not be committed if the version that has been modified is not the latest version. In this scenario, the version state is "Locally modified and stale" (![Locally Modified and Stale](https://nifi.apache.org/docs/nifi-docs/html/images/iconLocallyModifiedStale.png)).

### Change Version

To change the version of a flow, right-click on the versioned process group and select "Version→Change version".

![Change Version](https://nifi.apache.org/docs/nifi-docs/html/images/change-version.png)

In the Change Version dialog, select the desired version and select "Change":

![Change Version Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/change-version-dialog.png)

The version of the flow is changed:

![Flow Version Changed](https://nifi.apache.org/docs/nifi-docs/html/images/flow-version-changed.png)
...
![Stop Version Control](https://nifi.apache.org/docs/nifi-docs/html/images/stop-version-control.png)

In the Stop Version Control dialog, select "Disconnect".

![Stop Version Control Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/stop-version-control-dialog.png)

The removal of the process group from version control is confirmed.

![Disconnect Confirmation Dialog](https://nifi.apache.org/docs/nifi-docs/html/images/disconnect-dialog.png)

![Version Control Stopped on Process Group](https://nifi.apache.org/docs/nifi-docs/html/images/process-group-version-control-stopped.png)
...
![Root Process Group Defined Variable](https://nifi.apache.org/docs/nifi-docs/html/images/rpg-variable.png)

A process group PG1 is created:

![PG1 Process Group](https://nifi.apache.org/docs/nifi-docs/html/images/PG1_process_group.png)

The GetFile processor in PG1 references the variable "RPG_Var":

![PG1 References RPG Variable](http://localhost:8080/nifi-docs/html/images/PG1_variable_reference.png)

PG1 is saved as a versioned flow:

![PG1 Versioned Flow](https://nifi.apache.org/docs/nifi-docs/html/images/PG1_versioned_flow.png)

If PG1 versioned flow is imported into this same NiFi instance:

![PG1 Imported to Same NiFi](https://nifi.apache.org/docs/nifi-docs/html/images/PG1_imported_same.png)

the added GetFile processor will also reference the "RPG_Var" variable that exists in the root process group:

![Both PG1 Reference RPG Variable](https://nifi.apache.org/docs/nifi-docs/html/images/PG1_variable_ref_2.png)

If PG1 versioned flow is imported into a different NiFi instance where "RPG_Var" does not exist:

![PG1 Imported to Different NiFi](https://nifi.apache.org/docs/nifi-docs/html/images/PG1_imported_diff.png)

A "RPG_Var" variable is created in the PG1 process group:

![PG1 References PG Variable Copy](https://nifi.apache.org/docs/nifi-docs/html/images/PG1_variable_ref_PG.png)
...
- "sys_admin" has access to all restricted components.
  ...
  ![Sys_admin Restricted Component Access Policy](https://nifi.apache.org/docs/nifi-docs/html/images/sys_admin-restricted-component-access-policy.png)

- "test_user" has access to restricted components requiring 'read filesystem' and 'write filesystem'.

  ![Test_user Restricted Component Read Filesystem](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-restricted-component-read-filesystem.png)

  ![Test_user Restricted Component Write Filesystem](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-restricted-component-write-filesystem.png)
...
![KeytabCredentialsService Controller Service RPG Level](https://nifi.apache.org/docs/nifi-docs/html/images/keytabCredentialsService-rpg.png)

KeytabCredentialService controller service is a restricted component that requires 'access keytab' permissions:

![KeytabCredentialService Required Permissions](https://nifi.apache.org/docs/nifi-docs/html/images/keytabcredentialsservice-permissions.png)

Sys_admin creates a process group ABC containing a flow with GetFile and PutHDFS processors:

![Restricted Component Flow](https://nifi.apache.org/docs/nifi-docs/html/images/abc-restricted-component-flow.png)

GetFile processor is a restricted component that requires 'write filesystem' and 'read filesystem' permissions:

![GetFile Required Permissions](https://nifi.apache.org/docs/nifi-docs/html/images/getfile-permissions.png)

PutHDFS is a restricted component that requires 'write filesystem' permissions:

![PutHDFS Required Permissions](https://nifi.apache.org/docs/nifi-docs/html/images/puthdfs-permissions.png)

The PutHDFS processor is configured to use the root process group level KeytabCredentialsService controller service:

![PutHDFS Properties](https://nifi.apache.org/docs/nifi-docs/html/images/puthdfs-properties.png)

Sys_admin saves the process group as a versioned flow:

![ABC Versioned Flow](https://nifi.apache.org/docs/nifi-docs/html/images/abc-versioned-flow.png)

Test_user changes the flow by removing the KeytabCredentialsService controller service:

![PutHDFS No Kerberos CS](https://nifi.apache.org/docs/nifi-docs/html/images/puthdfs-no-kerberosCS.png)

If test_user chooses to revert this change:

![img](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-revert-local-changes.png)

the revert is successful:

![Revert Local Changes Successful](https://nifi.apache.org/docs/nifi-docs/html/images/revert-success.png)

Additionally, if test_user chooses to import the ABC versioned flow:

![Test_user Import Flow](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-import-abc-flow.png)

The import is successful:

![Test_user Import Successful](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-import-success.png)
...
![XYZ Process Group](https://nifi.apache.org/docs/nifi-docs/html/images/xyz-process-group.png)

Sys_admin creates a KeytabCredentialsService controller service at the process group level:

![KeytabCredentialsService Controller Service PG Level](https://nifi.apache.org/docs/nifi-docs/html/images/keytabCredentialsService-pg.png)

The same GetFile and PutHDFS flow is created in the process group:

![XYZ Versioned Flow](https://nifi.apache.org/docs/nifi-docs/html/images/xyz-flow.png)

However, PutHDFS now references the process group level controller service:

![PutHDFS Properties](https://nifi.apache.org/docs/nifi-docs/html/images/puthdfs-properties_2.png)
...
![Test_user Revert Local Changes](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-revert-local-changes-2.png)
...
![Revert Local Changes Fails](https://nifi.apache.org/docs/nifi-docs/html/images/revert-failure.png)

Similarly, if test_user tries to import the XYZ versioned flow:

![Test_user Import Flow](https://nifi.apache.org/docs/nifi-docs/html/images/test_user-import-xyz-flow.png)

The import fails:

![XYZ Import Fails](https://nifi.apache.org/docs/nifi-docs/html/images/import-xyz-flow-fails.png)
...
To create a Template, select the components that are to be a part of the template, and then click the "Create Template" ( ![Create Template](https://nifi.apache.org/docs/nifi-docs/html/images/iconNewTemplate.png) ) button in the Operate Palette (See [NiFi User Interface](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#User_Interface) for more information on the Operate Palette).
...
From the Operate Palette, click the "Upload Template" ( ![Upload Template](https://nifi.apache.org/docs/nifi-docs/html/images/iconUploadTemplate.png) ) button (see [NiFi User Interface](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#User_Interface) for more information on the Operate Palette). This will display the Upload Template dialog. Click the find icon and use the File Selection dialog to choose which template file to upload. Select the file and click Open. Clicking the "Upload" button will attempt to import the Template into this instance of NiFi. The Upload Template dialog will update to show "Success" or an error message if there was a problem importing the template.

### Instantiating a Template

Once a Template has been created (see [Creating a Template](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Create_Template)) or imported (see [Importing a Template](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Import_Template)), it is ready to be instantiated, or added to the canvas. This is accomplished by dragging the Template icon ( ![Template](https://nifi.apache.org/docs/nifi-docs/html/images/iconTemplate.png) ) from the Components Toolbar (see [NiFi User Interface](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#User_Interface)) onto the canvas.
...
![Provenance Table](https://nifi.apache.org/docs/nifi-docs/html/images/provenance-annotated.png)
...
![Search Events](https://nifi.apache.org/docs/nifi-docs/html/images/search-events.png)
...
![Search for RECEIVE Event](https://nifi.apache.org/docs/nifi-docs/html/images/search-receive-event-abc.png)

### Details of an Event

In the far-left column of the Data Provenance page, there is a View Details icon for each event (![Details](https://nifi.apache.org/docs/nifi-docs/html/images/iconDetails.png)). Clicking this button opens a dialog window with three tabs: Details, Attributes, and Content.
...
It is often useful to see a graphical representation of the lineage or path a FlowFile took within the dataflow. To see a FlowFile’s lineage, click on the "Show Lineage" icon ( ![Show Lineage](https://nifi.apache.org/docs/nifi-docs/html/images/iconLineage.png) ) in the far-right column of the Data Provenance table. This opens a graph displaying the FlowFile ( ![FlowFile](https://nifi.apache.org/docs/nifi-docs/html/images/lineage-flowfile.png) ) and the various processing events that have occurred. The selected event will be highlighted in red. It is possible to right-click or double-click on any event to see that event’s details (see [Details of an Event](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#event_details)). To see how the lineage evolved over time, click the slider at the bottom-left of the window and move it to the left to see the state of the lineage at earlier stages in the dataflow.
...
![Find Parents](https://nifi.apache.org/docs/nifi-docs/html/images/find-parents.png)
...
![Parent Found](https://nifi.apache.org/docs/nifi-docs/html/images/parent-found.png)
...
![Expand Event](https://nifi.apache.org/docs/nifi-docs/html/images/expand-event.png)

Once "Expand" is selected, the graph is re-drawn to show the children and their lineage.

![Expanded Events](https://nifi.apache.org/docs/nifi-docs/html/images/expanded-events.png)
...
**Note:** Detailed descriptions for each of these properties can be found in [System Properties](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html#system_properties).
...
The fully qualified class `org.apache.nifi.provenance.EncryptedWriteAheadProvenanceRepository` is specified as the provenance repository implementation in `nifi.properties` as the value of `nifi.provenance.repository.implementation`. In addition, [new properties](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html#encrypted-write-ahead-provenance-repository-properties) must be populated to allow successful initialization.

##### StaticKeyProvider

The `StaticKeyProvider` implementation defines keys directly in `nifi.properties`. Individual keys are provided in hexadecimal encoding. The keys can also be encrypted like any other sensitive property in `nifi.properties` using the [`./encrypt-config.sh`](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html#encrypt-config_tool) tool in the NiFi Toolkit.
...
![Encrypted provenance repository file on disk](https://nifi.apache.org/docs/nifi-docs/html/images/encrypted-wapr-hex.png)
...
- Switching between unencrypted and encrypted repositories
  ...
  - There is also a future effort to provide a standalone tool in NiFi Toolkit to encrypt/decrypt an existing provenance repository to make the transition easier. The translation process could take a long time depending on the size of the existing repository, and being able to perform this task outside of application startup would be valuable ([NIFI-3723](https://issues.apache.org/jira/browse/NIFI-3723)).
...
## 14 Other Management Features

In addition to the Summary Page, Data Provenance Page, Template Management Page, and Bulletin Board Page, there are other tools in the Global Menu (see [NiFi User Interface](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#User_Interface)) that are useful to the DFM. Select Flow Configuration History to view all the changes that have been made to the dataflow. The history can aid in troubleshooting, such as if a recent change to the dataflow has caused a problem and needs to be fixed. The DFM can see what changes have been made and adjust the flow as needed to fix the problem. While NiFi does not have an "undo" feature, the DFM can make new changes to the dataflow that will fix the problem.

Two other tools in the Global Menu are Controller Settings and Users. The Controller Settings page provides the ability to change the name of the NiFi instance, add comments describing the NiFi instance, and set the maximum number of threads that are available to the application. It also provides tabs where DFMs may add and configure [Controller Services](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Controller_Services)and [Reporting Tasks](https://nifi.apache.org/docs/nifi-docs/html/user-guide.html#Reporting_Tasks). The Users page is used to manage user access, which is described in the [System Administrator’s Guide](https://nifi.apache.org/docs/nifi-docs/html/administration-guide.html).
